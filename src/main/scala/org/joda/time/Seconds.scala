...
object Seconds {
  ...
  val ZERO: Seconds = new Seconds(0)
  /** Constant representing one second. */
  val ONE: Seconds = new Seconds(1)
  /** Constant representing two seconds. */
  val TWO: Seconds = new Seconds(2)
  /** Constant representing three seconds. */
  val THREE: Seconds = new Seconds(3)
  /** Constant representing the maximum number of seconds that can be stored in this object. */
  val MAX_VALUE: Seconds = new Seconds(Integer.MAX_VALUE)
  /** Constant representing the minimum number of seconds that can be stored in this object. */
  val MIN_VALUE: Seconds = new Seconds(Integer.MIN_VALUE)
  /** The paser to use for this class. */
  private val PARSER: PeriodFormatter = ISOPeriodFormat.standard.withParseType(PeriodType.seconds)
  ...
  def secondsBetween(start: ReadableInstant, end: ReadableInstant): Seconds = {
    val amount: Int = BaseSingleFieldPeriod.between(start, end, DurationFieldType.seconds)
  ...
  def secondsBetween(start: ReadablePartial, end: ReadablePartial): Seconds = {
    if (start.isInstanceOf[LocalTime] && end.isInstanceOf[LocalTime]) {
      ...
      val seconds: Int = chrono.seconds.getDifference((end.asInstanceOf[LocalTime]).getLocalMillis, (start.asInstanceOf[LocalTime]).getLocalMillis)
  ...
  def secondsIn(interval: ReadableInterval): Seconds = {
    ...
    val amount: Int = BaseSingleFieldPeriod.between(interval.getStart, interval.getEnd, DurationFieldType.seconds)
    return Seconds.seconds(amount)
  }
  ...
  def standardSecondsIn(period: ReadablePeriod): Seconds = {
    val amount: Int = BaseSingleFieldPeriod.standardPeriodIn(period, DateTimeConstants.MILLIS_PER_SECOND)
    return Seconds.seconds(amount)
  }
   ...
   */
  @FromString def parseSeconds(periodStr: String): Seconds = {
    if (periodStr == null) {
      return Seconds.ZERO
    }
...
@SerialVersionUID(87525275727380862L)
final class Seconds extends BaseSingleFieldPeriod {
  /**
