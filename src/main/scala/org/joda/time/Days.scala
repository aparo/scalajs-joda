...
object Days {
  ...
  def daysBetween(start: ReadableInstant, end: ReadableInstant): Days = {
    val amount: Int = BaseSingleFieldPeriod.between(start, end, DurationFieldType.days)
  ...
  def daysBetween(start: ReadablePartial, end: ReadablePartial): Days = {
    if (start.isInstanceOf[LocalDate] && end.isInstanceOf[LocalDate]) {
      ...
      val days: Int = chrono.days.getDifference((end.asInstanceOf[LocalDate]).getLocalMillis, (start.asInstanceOf[LocalDate]).getLocalMillis)
  ...
  def daysIn(interval: ReadableInterval): Days = {
    ...
    val amount: Int = BaseSingleFieldPeriod.between(interval.getStart, interval.getEnd, DurationFieldType.days)
    return Days.days(amount)
  }
  ...
  def standardDaysIn(period: ReadablePeriod): Days = {
    val amount: Int = BaseSingleFieldPeriod.standardPeriodIn(period, DateTimeConstants.MILLIS_PER_DAY)
    return Days.days(amount)
  }
   ...
   */
  @FromString def parseDays(periodStr: String): Days = {
    if (periodStr == null) {
      return Days.ZERO
    }
...
@SerialVersionUID(87525275727380865L)
final class Days extends BaseSingleFieldPeriod {
  /**
