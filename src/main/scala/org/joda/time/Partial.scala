 ...
 * and always matches.
 * <p>
 * Calculations on Partial are performed using a {@link org.joda.Chronology}.
...
object Partial {
  ...
  class Property extends AbstractPartialFieldProperty with Serializable {
    /** The partial */
    private final val iPartial: Partial = null
    /** The field index */
     ...
     */
    private[time] def this(partial: Partial, fieldIndex: Int) {
      this()
      `super`
      iPartial = partial
     ...
     */
    def addToCopy(valueToAdd: Int): Partial = {
      var newValues: Array[Int] = iPartial.getValues
     ...
     * @throws IllegalArgumentException if the value isn't valid
     */
    def addWrapFieldToCopy(valueToAdd: Int): Partial = {
      var newValues: Array[Int] = iPartial.getValues
     ...
     */
    def setCopy(text: String, locale: Locale): Partial = {
      var newValues: Array[Int] = iPartial.getValues
     ...
     * @throws IllegalArgumentException if the text value isn't valid
     */
    def setCopy(text: String): Partial = {
...
final class Partial extends AbstractPartial with ReadablePartial with Serializable {
  ...
  def this() {
    this()
    `this`(null.asInstanceOf[Chronology])
  }
   ...
   */
  def this(chrono: Chronology) {
    this()
    `super`
    iChronology = DateTimeUtils.getChronology(chrono).withUTC
    iTypes = new Array[DateTimeFieldType](0)
    iValues = new Array[Int](0)
  }
   ...
   */
  def this(`type`: DateTimeFieldType, value: Int) {
    this()
   ...
   * @throws IllegalArgumentException if the type or value is invalid
   */
  def this(`type`: DateTimeFieldType, value: Int, chronology: Chronology) {
    this()
    `super`
    ...
    iTypes = Array[DateTimeFieldType](`type`)
    iValues = Array[Int](value)
   ...
   */
  def this(types: Array[DateTimeFieldType], values: Array[Int]) {
    this()
   ...
   * @throws IllegalArgumentException if the types or values are invalid
   */
  def this(types: Array[DateTimeFieldType], values: Array[Int], chronology: Chronology) {
    this()
    `super`
    chronology = DateTimeUtils.getChronology(chronology).withUTC
    iChronology = chronology
    ...
    var lastUnitField: DurationField = null
    {
      var i: Int = 0
      while (i < types.length) {
        {
          val loopType: DateTimeFieldType = types(i)
          val loopUnitField: DurationField = loopType.getDurationType.getField(iChronology)
          if (i > 0) {
            ...
            val compare: Int = lastUnitField.compareTo(loopUnitField)
            if (compare < 0) {
              throw new IllegalArgumentException("Types array must be in order largest-smallest: " + types(i - 1).getName + " < " + loopType.getName)
            }
            else if (compare == 0) {
              if (lastUnitField == loopUnitField) {
                val lastRangeType: DurationFieldType = types(i - 1).getRangeDurationType
                val loopRangeType: DurationFieldType = loopType.getRangeDurationType
                if (lastRangeType == null) {
                  ...
                  val lastRangeField: DurationField = lastRangeType.getField(iChronology)
                  val loopRangeField: DurationField = loopRangeType.getField(iChronology)
                ...
                if (lastUnitField.isSupported && lastUnitField.getType ne DurationFieldType.YEARS_TYPE) {
                  throw new IllegalArgumentException("Types array must be in order largest-smallest," + " for year-based fields, years is defined as being largest: " + types(i - 1).getName + " < " + loopType.getName)
                }
              }
            }
          }
    ...
    iTypes = types.clone.asInstanceOf[Array[DateTimeFieldType]]
    chronology.validate(this, values)
    iValues = values.clone.asInstanceOf[Array[Int]]
  }
   ...
   */
  def this(partial: ReadablePartial) {
    this()
    `super`
    if (partial == null) {
    ...
    iTypes = new Array[DateTimeFieldType](partial.size)
    iValues = new Array[Int](partial.size)
    {
      var i: Int = 0
   ...
   * @throws IllegalArgumentException if the types or values are invalid
   */
  private[time] def this(partial: Partial, values: Array[Int]) {
    this()
    `super`
   ...
   * @throws IllegalArgumentException if the types or values are invalid
   */
  private[time] def this(chronology: Chronology, types: Array[DateTimeFieldType], values: Array[Int]) {
    this()
    `super`
    iChronology = chronology
    iTypes = types
  ...
  override def getFieldTypes: Array[DateTimeFieldType] = {
    return iTypes.clone.asInstanceOf[Array[DateTimeFieldType]]
  }
  ...
  override def getValues: Array[Int] = {
    return iValues.clone.asInstanceOf[Array[Int]]
  }
   ...
   */
  def withChronologyRetainFields(newChronology: Chronology): Partial = {
    ...
    if (newChronology eq getChronology) {
      ...
      val newPartial: Partial = new Partial(newChronology, iTypes, iValues)
   ...
   */
  def `with`(fieldType: DateTimeFieldType, value: Int): Partial = {
    if (fieldType == null) {
      throw new IllegalArgumentException("The field type must not be null")
    }
    val index: Int = indexOf(fieldType)
    if (index == -1) {
      val newTypes: Array[DateTimeFieldType] = new Array[DateTimeFieldType](iTypes.length + 1)
      val newValues: Array[Int] = new Array[Int](newTypes.length)
      var i: Int = 0
      val unitField: DurationField = fieldType.getDurationType.getField(iChronology)
      if (unitField.isSupported) {
        while (i < iTypes.length) {
          {
            val loopType: DateTimeFieldType = iTypes(i)
            val loopUnitField: DurationField = loopType.getDurationType.getField(iChronology)
            if (loopUnitField.isSupported) {
              val compare: Int = unitField.compareTo(loopUnitField)
              if (compare > 0) {
                ...
                val rangeField: DurationField = fieldType.getRangeDurationType.getField(iChronology)
                val loopRangeField: DurationField = loopType.getRangeDurationType.getField(iChronology)
      ...
      val newPartial: Partial = new Partial(newTypes, newValues, iChronology)
   ...
   */
  def without(fieldType: DateTimeFieldType): Partial = {
    val index: Int = indexOf(fieldType)
    if (index != -1) {
      val newTypes: Array[DateTimeFieldType] = new Array[DateTimeFieldType](size - 1)
      val newValues: Array[Int] = new Array[Int](size - 1)
      ...
      val newPartial: Partial = new Partial(iChronology, newTypes, newValues)
      iChronology.validate(newPartial, newValues)
      return newPartial
    }
    return this
   ...
   * @throws IllegalArgumentException if the value is null or invalid
   */
  def withField(fieldType: DateTimeFieldType, value: Int): Partial = {
   ...
   */
  def withFieldAdded(fieldType: DurationFieldType, amount: Int): Partial = {
    val index: Int = indexOfSupported(fieldType)
    if (amount == 0) {
      return this
    }
   ...
   * @throws ArithmeticException if the new datetime exceeds the capacity
   */
  def withFieldAddWrapped(fieldType: DurationFieldType, amount: Int): Partial = {
    val index: Int = indexOfSupported(fieldType)
    if (amount == 0) {
      return this
    }
   ...
   * @throws ArithmeticException if the new datetime exceeds the capacity
   */
  def withPeriodAdded(period: ReadablePeriod, scalar: Int): Partial = {
   ...
   */
  def plus(period: ReadablePeriod): Partial = {
   ...
   * @throws ArithmeticException if the new datetime exceeds the capacity of a long
   */
  def minus(period: ReadablePeriod): Partial = {
    return withPeriodAdded(period, -1)
  }
   ...
   */
  def property(`type`: DateTimeFieldType): Partial.Property = {
    return new Partial.Property(this, indexOfSupported(`type`))
  }
   ...
   */
  def isMatch(instant: ReadableInstant): Boolean = {
    val millis: Long = DateTimeUtils.getInstantMillis(instant)
    val chrono: Chronology = DateTimeUtils.getInstantChronology(instant)
    {
      var i: Int = 0
      while (i < iTypes.length) {
        {
          val value: Int = iTypes(i).getField(chrono).get(millis)
          if (value != iValues(i)) {
            return false
          }
        }
   ...
   * @since 1.5
   */
  def isMatch(partial: ReadablePartial): Boolean = {
    if (partial == null) {
      throw new IllegalArgumentException("The partial must not be null")
    }
    {
      ...
      while (i < iTypes.length) {
        {
          val value: Int = partial.get(iTypes(i))
          if (value != iValues(i)) {
            return false
          }
        }
  ...
  def getFormatter: DateTimeFormatter = {
    var f: Array[DateTimeFormatter] = iFormatter
    if (f == null) {
      if (size == 0) {
        return null
      }
      f = new Array[DateTimeFormatter](2)
      try {
        val list: List[DateTimeFieldType] = new ArrayList[DateTimeFieldType](Arrays.asList(iTypes))
  ...
  def toStringList: String = {
    val size: Int = size
    val buf: StringBuilder = new StringBuilder(20 * size)
    buf.append('[')
    {
      var i: Int = 0
      while (i < size) {
