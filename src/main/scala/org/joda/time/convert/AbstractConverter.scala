 ...
 * @since 1.0
 */
abstract class AbstractConverter extends Converter {
  /**
  ...
  def getPartialValues(fieldSource: ReadablePartial, `object`: AnyRef, chrono: Chronology): Array[Int] = {
    val instant: Long = getInstantMillis(`object`, chrono)
   ...
   * @since 1.3
   */
  def getPartialValues(fieldSource: ReadablePartial, `object`: AnyRef, chrono: Chronology, parser: DateTimeFormatter): Array[Int] = {
  ...
  override def toString: String = {
    return "Converter[" + (if (getSupportedType == null) "null" else getSupportedType.getName) + "]"
  }
}