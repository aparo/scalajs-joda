...
object Weeks {
  ...
  val ZERO: Weeks = new Weeks(0)
  /** Constant representing one week. */
  val ONE: Weeks = new Weeks(1)
  /** Constant representing two weeks. */
  val TWO: Weeks = new Weeks(2)
  /** Constant representing three weeks. */
  val THREE: Weeks = new Weeks(3)
  /** Constant representing the maximum number of weeks that can be stored in this object. */
  val MAX_VALUE: Weeks = new Weeks(Integer.MAX_VALUE)
  /** Constant representing the minimum number of weeks that can be stored in this object. */
  val MIN_VALUE: Weeks = new Weeks(Integer.MIN_VALUE)
  /** The paser to use for this class. */
  private val PARSER: PeriodFormatter = ISOPeriodFormat.standard.withParseType(PeriodType.weeks)
  ...
  def weeksBetween(start: ReadableInstant, end: ReadableInstant): Weeks = {
    val amount: Int = BaseSingleFieldPeriod.between(start, end, DurationFieldType.weeks)
  ...
  def weeksBetween(start: ReadablePartial, end: ReadablePartial): Weeks = {
    if (start.isInstanceOf[LocalDate] && end.isInstanceOf[LocalDate]) {
      ...
      val weeks: Int = chrono.weeks.getDifference((end.asInstanceOf[LocalDate]).getLocalMillis, (start.asInstanceOf[LocalDate]).getLocalMillis)
  ...
  def weeksIn(interval: ReadableInterval): Weeks = {
    ...
    val amount: Int = BaseSingleFieldPeriod.between(interval.getStart, interval.getEnd, DurationFieldType.weeks)
    return Weeks.weeks(amount)
  }
  ...
  def standardWeeksIn(period: ReadablePeriod): Weeks = {
    val amount: Int = BaseSingleFieldPeriod.standardPeriodIn(period, DateTimeConstants.MILLIS_PER_WEEK)
    return Weeks.weeks(amount)
  }
   ...
   */
  @FromString def parseWeeks(periodStr: String): Weeks = {
    if (periodStr == null) {
      return Weeks.ZERO
    }
...
@SerialVersionUID(87525275727380866L)
final class Weeks extends BaseSingleFieldPeriod {
  /**
