 ...
 * @since 1.0
 */
abstract class AbstractPartialFieldProperty {
  /**
   * Constructor.
   */
  protected def this() {
  ...
  override def equals(`object`: AnyRef): Boolean = {
    ...
    if (`object`.isInstanceOf[AbstractPartialFieldProperty] == false) {
      return false
    }
    val other: AbstractPartialFieldProperty = `object`.asInstanceOf[AbstractPartialFieldProperty]
    return get == other.get && getFieldType eq other.getFieldType && FieldUtils.equals(getReadablePartial.getChronology, other.getReadablePartial.getChronology)
  }
