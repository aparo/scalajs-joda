...
abstract class AbstractReadableInstantFieldProperty extends Serializable {
   ...
   */
  def getFieldType: DateTimeFieldType = {
   ...
   */
  def getMinimumValueOverall: Int = {
   ...
   */
  def getMaximumValueOverall: Int = {
  ...
  def toInterval: Interval = {
    val field: DateTimeField = getField
    val start: Long = field.roundFloor(getMillis)
    val end: Long = field.add(start, 1)
  ...
  def compareTo(instant: ReadableInstant): Int = {
    ...
    val thisValue: Int = get
    val otherValue: Int = instant.get(getFieldType)
  ...
  def compareTo(partial: ReadablePartial): Int = {
    ...
    val thisValue: Int = get
    val otherValue: Int = partial.get(getFieldType)
    if (thisValue < otherValue) {
      return -1
    }
    else if (thisValue > otherValue) {
  ...
  override def equals(`object`: AnyRef): Boolean = {
    ...
    if (`object`.isInstanceOf[AbstractReadableInstantFieldProperty] == false) {
      return false
    }
    val other: AbstractReadableInstantFieldProperty = `object`.asInstanceOf[AbstractReadableInstantFieldProperty]
    return get == other.get && (getFieldType == other.getFieldType) && FieldUtils.equals(getChronology, other.getChronology)
  }
