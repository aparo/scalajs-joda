...
object Duration {
   ...
   * @since 2.0
   */
  @FromString def parse(str: String): Duration = {
...
@SerialVersionUID(2471658376918L)
final class Duration extends BaseDuration with ReadableDuration with Serializable {
  /**
   ...
   */
  override def toDuration: Duration = {
    return this
  }
  ...
  def toStandardHours: Hours = {
    val hours: Long = getStandardHours
  ...
  def toStandardMinutes: Minutes = {
    val minutes: Long = getStandardMinutes
  ...
  def toStandardSeconds: Seconds = {
    val seconds: Long = getStandardSeconds
  ...
  def withDurationAdded(durationToAdd: Long, scalar: Int): Duration = {
    ...
    val add: Long = FieldUtils.safeMultiply(durationToAdd, scalar)
    val duration: Long = FieldUtils.safeAdd(getMillis, add)
    return new Duration(duration)
  }
   ...
   * @return the new duration instance
   */
  def minus(amount: Long): Duration = {
    return withDurationAdded(amount, -1)
  }
