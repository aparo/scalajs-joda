...
abstract class BasePartial extends AbstractPartial with ReadablePartial with Serializable {
   ...
   * @param chronology  the chronology, null means ISOChronology in the default zone
   */
  protected def this(instant: Long, chronology: Chronology) {
    this()
    `super`
  ...
  protected def setValue(index: Int, value: Int) {
    val field: DateTimeField = getField(index)
    val values: Array[Int] = field.set(this, index, iValues, value)
