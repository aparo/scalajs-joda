...
abstract class BaseDuration extends AbstractDuration with ReadableDuration with Serializable {
  /** The duration length */
  @volatile
  ...
  protected def this(duration: AnyRef) {
    this()
    `super`
    val converter: DurationConverter = ConverterManager.getInstance.getDurationConverter(duration)
   ...
   * @return a Period created using the millisecond duration from this instance
   */
  def toPeriod(chrono: Chronology): Period = {
