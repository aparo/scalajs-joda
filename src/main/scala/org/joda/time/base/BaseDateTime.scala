...
abstract class BaseDateTime extends AbstractDateTime with ReadableDateTime with Serializable {
  /** The millis from 1970-01-01T00:00:00Z */
  @volatile
  private var iMillis: Long = 0L
  ...
  def this(instant: AnyRef, zone: DateTimeZone) {
    ...
    val chrono: Chronology = checkChronology(converter.getChronology(instant, zone))
    iChronology = chrono
  ...
  def this(year: Int, monthOfYear: Int, dayOfMonth: Int, hourOfDay: Int, minuteOfHour: Int, secondOfMinute: Int, millisOfSecond: Int, chronology: Chronology) {
    ...
    `super`
    iChronology = checkChronology(chronology)
    val instant: Long = iChronology.getDateTimeMillis(year, monthOfYear, dayOfMonth, hourOfDay, minuteOfHour, secondOfMinute, millisOfSecond)
    iMillis = checkInstant(instant, iChronology)
  }
