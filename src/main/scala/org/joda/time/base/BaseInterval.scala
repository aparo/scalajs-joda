...
abstract class BaseInterval extends AbstractInterval with ReadableInterval with Serializable {
  /** The chronology of the interval */
  @volatile
  private var iChronology: Chronology = null
  /** The start of the interval */
  @volatile
  private var iStartMillis: Long = 0L
  /** The end of the interval */
  @volatile
  private var iEndMillis: Long = 0L
  ...
  protected def this(start: ReadableInstant, end: ReadableInstant) {
    ...
    if (start == null && end == null) {
      iStartMillis = ({
        iEndMillis = DateTimeUtils.currentTimeMillis; iEndMillis
      })
  ...
  protected def this(period: ReadablePeriod, end: ReadableInstant) {
    this()
    `super`
    val chrono: Chronology = DateTimeUtils.getInstantChronology(end)
    iChronology = chrono
    iEndMillis = DateTimeUtils.getInstantMillis(end)
    if (period == null) {
  ...
  protected def this(interval: AnyRef, chrono: Chronology) {
    this()
    `super`
    val converter: IntervalConverter = ConverterManager.getInstance.getIntervalConverter(interval)
    if (converter.isReadableInterval(interval, chrono)) {
      val input: ReadableInterval = interval.asInstanceOf[ReadableInterval]
      ...
      val mi: MutableInterval = new MutableInterval
