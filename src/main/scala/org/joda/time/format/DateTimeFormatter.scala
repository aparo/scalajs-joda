...
class DateTimeFormatter {
  ...
  private final val iPrinter: InternalPrinter = null
  /** The internal parser used to output the datetime. */
  private final val iParser: InternalParser = null
  /** The locale to use for printing and parsing. */
  private final val iLocale: Locale = null
  /** Whether the offset is parsed. */
  private final val iOffsetParsed: Boolean = false
  ...
  private final val iPivotYear: Integer = null
   ...
   * @param parser  the internal parser, null if cannot parse
   */
  private[format] def this(printer: InternalPrinter, parser: InternalParser) {
    this()
    `super`
    iPrinter = printer
    iParser = parser
   ...
   * Constructor.
   */
  private def this(printer: InternalPrinter, parser: InternalParser, locale: Locale, offsetParsed: Boolean, chrono: Chronology, zone: DateTimeZone, pivotYear: Integer, defaultYear: Int) {
    this()
    `super`
    iPrinter = printer
    iParser = parser
   ...
   * @return the internal printer; is null if printing not supported
   */
  private[format] def getPrinter0: InternalPrinter = {
    return iPrinter
  }
  ...
  private[format] def getParser0: InternalParser = {
    return iParser
  }
  ...
  def withLocale(locale: Locale): DateTimeFormatter = {
    if (locale eq getLocale || (locale != null && (locale == getLocale))) {
      return this
    }
   ...
   */
  @deprecated def getChronolgy: Chronology = {
    return iChrono
  }
   ...
   * @since 2.0
   */
  def withZoneUTC: DateTimeFormatter = {
  ...
  def withPivotYear(pivotYear: Integer): DateTimeFormatter = {
    if (iPivotYear eq pivotYear || (iPivotYear != null && (iPivotYear == pivotYear))) {
      return this
    }
  ...
  def printTo(appendable: Appendable, partial: ReadablePartial) {
    val printer: InternalPrinter = requirePrinter
    if (partial == null) {
  ...
  def print(instant: ReadableInstant): String = {
    val buf: StringBuilder = new StringBuilder(requirePrinter.estimatePrintedLength)
    try {
      printTo(buf.asInstanceOf[Appendable], instant)
    }
    catch {
  ...
  private def printTo(appendable: Appendable, instant: Long, chrono: Chronology) {
    ...
    var zone: DateTimeZone = chrono.getZone
    var offset: Int = zone.getOffset(instant)
    var adjustedInstant: Long = instant + offset
  ...
  private def requirePrinter: InternalPrinter = {
    val printer: InternalPrinter = iPrinter
    if (printer == null) {
  ...
  def parseInto(instant: ReadWritableInstant, text: String, position: Int): Int = {
    val parser: InternalParser = requireParser
    if (instant == null) {
      throw new IllegalArgumentException("Instant must not be null")
    }
    val instantMillis: Long = instant.getMillis
    var chrono: Chronology = instant.getChronology
    val defaultYear: Int = DateTimeUtils.getChronology(chrono).year.get(instantMillis)
    val instantLocal: Long = instantMillis + chrono.getZone.getOffset(instantMillis)
    chrono = selectChronology(chrono)
    val bucket: DateTimeParserBucket = new DateTimeParserBucket(instantLocal, chrono, iLocale, iPivotYear, defaultYear)
    val newPos: Int = parser.parseInto(bucket, text, position)
    instant.setMillis(bucket.computeMillis(false, text))
    if (iOffsetParsed && bucket.getOffsetInteger != null) {
      val parsedOffset: Int = bucket.getOffsetInteger
      val parsedZone: DateTimeZone = DateTimeZone.forOffsetMillis(parsedOffset)
  ...
  def parseMillis(text: String): Long = {
    val parser: InternalParser = requireParser
    val chrono: Chronology = selectChronology(iChrono)
    val bucket: DateTimeParserBucket = new DateTimeParserBucket(0, chrono, iLocale, iPivotYear, iDefaultYear)
  ...
  def parseLocalDateTime(text: String): LocalDateTime = {
    val parser: InternalParser = requireParser
    var chrono: Chronology = selectChronology(null).withUTC
    val bucket: DateTimeParserBucket = new DateTimeParserBucket(0, chrono, iLocale, iPivotYear, iDefaultYear)
    var newPos: Int = parser.parseInto(bucket, text, 0)
    if (newPos >= 0) {
      if (newPos >= text.length) {
        val millis: Long = bucket.computeMillis(true, text)
  ...
  def parseDateTime(text: String): DateTime = {
    val parser: InternalParser = requireParser
    var chrono: Chronology = selectChronology(null)
    val bucket: DateTimeParserBucket = new DateTimeParserBucket(0, chrono, iLocale, iPivotYear, iDefaultYear)
    var newPos: Int = parser.parseInto(bucket, text, 0)
    if (newPos >= 0) {
      if (newPos >= text.length) {
        ...
        var dt: DateTime = new DateTime(millis, chrono)
        if (iZone != null) {
  ...
  def parseMutableDateTime(text: String): MutableDateTime = {
    ...
    if (newPos >= 0) {
      if (newPos >= text.length) {
        ...
        val dt: MutableDateTime = new MutableDateTime(millis, chrono)
        if (iZone != null) {
          dt.setZone(iZone)
        }
        return dt
  ...
  private def requireParser: InternalParser = {
    val parser: InternalParser = iParser
    if (parser == null) {
