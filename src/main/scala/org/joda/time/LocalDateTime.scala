 ...
 * <p>
 * Calculations on LocalDateTime are performed using a {@link org.joda.Chronology}.
...
object LocalDateTime {
  ...
  private val MILLIS_OF_DAY: Int = 3
   ...
   * @since 2.0
   */
  @FromString def parse(str: String): LocalDateTime = {
  ...
  def fromCalendarFields(calendar: Calendar): LocalDateTime = {
    ...
    return new LocalDateTime((if (era == GregorianCalendar.AD) yearOfEra else 1 - yearOfEra), calendar.get(Calendar.MONTH) + 1, calendar.get(Calendar.DAY_OF_MONTH), calendar.get(Calendar.HOUR_OF_DAY), calendar.get(Calendar.MINUTE), calendar.get(Calendar.SECOND), calendar.get(Calendar.MILLISECOND))
  }
   ...
   * @throws IllegalArgumentException if the date is invalid for the ISO chronology
   */
  @SuppressWarnings(Array("deprecation")) def fromDateFields(date: Date): LocalDateTime = {
    if (date == null) {
    ...
    return new LocalDateTime(date.getYear + 1900, date.getMonth + 1, date.getDate, date.getHours, date.getMinutes, date.getSeconds, (((date.getTime % 1000).toInt) + 1000) % 1000)
  }
  ...
  final class Property extends AbstractReadableInstantFieldProperty {
    ...
    @transient
    private var iInstant: LocalDateTime = null
     ...
     */
    private[time] def this(instant: LocalDateTime, field: DateTimeField) {
      this()
      `super`
      iInstant = instant
      iField = field
     ...
     * @throws IllegalArgumentException if the text value isn't valid
     */
    def setCopy(text: String): LocalDateTime = {
     ...
     */
    def withMaximumValue: LocalDateTime = {
     ...
     */
    def withMinimumValue: LocalDateTime = {
...
@SerialVersionUID(-268716875315837168L)
final class LocalDateTime extends BaseLocal with ReadablePartial with Serializable {
  ...
  def this(instant: AnyRef, zone: DateTimeZone) {
    ...
    chronology = DateTimeUtils.getChronology(chronology)
    iChronology = chronology.withUTC
    val values: Array[Int] = converter.getPartialValues(this, instant, chronology, ISODateTimeFormat.localDateOptionalTimeParser)
  ...
  def this(year: Int, monthOfYear: Int, dayOfMonth: Int, hourOfDay: Int, minuteOfHour: Int, secondOfMinute: Int, millisOfSecond: Int, chronology: Chronology) {
    ...
    val instant: Long = chronology.getDateTimeMillis(year, monthOfYear, dayOfMonth, hourOfDay, minuteOfHour, secondOfMinute, millisOfSecond)
  ...
  protected def getField(index: Int, chrono: Chronology): DateTimeField = {
    index match {
      case LocalDateTime.YEAR =>
        return chrono.year
      case LocalDateTime.MONTH_OF_YEAR =>
        return chrono.monthOfYear
      case LocalDateTime.DAY_OF_MONTH =>
        return chrono.dayOfMonth
      case LocalDateTime.MILLIS_OF_DAY =>
  ...
  override def equals(partial: AnyRef): Boolean = {
    ...
    if (partial.isInstanceOf[LocalDateTime]) {
      val other: LocalDateTime = partial.asInstanceOf[LocalDateTime]
  ...
  def toDateTime: DateTime = {
    return toDateTime(null.asInstanceOf[DateTimeZone])
  }
  ...
  def toDateTime(zone: DateTimeZone): DateTime = {
    ...
    val chrono: Chronology = iChronology.withZone(zone)
  ...
  @SuppressWarnings(Array("deprecation")) def toDate: Date = {
    ...
    val date: Date = new Date(getYear - 1900, getMonthOfYear - 1, dom, getHourOfDay, getMinuteOfHour, getSecondOfMinute)
  ...
  private def correctDstTransition(date: Date, timeZone: TimeZone): Date = {
    ...
    var check: LocalDateTime = LocalDateTime.fromCalendarFields(calendar)
    if (check.isBefore(this)) {
      ...
      val earlier: Calendar = Calendar.getInstance(timeZone)
   ...
   */
  private[time] def withLocalMillis(newMillis: Long): LocalDateTime = {
    return (if (newMillis == getLocalMillis) this else new LocalDateTime(newMillis, getChronology))
  }
  ...
  def withDurationAdded(durationToAdd: ReadableDuration, scalar: Int): LocalDateTime = {
    ...
    val instant: Long = getChronology.add(getLocalMillis, durationToAdd.getMillis, scalar)
    return withLocalMillis(instant)
  }
   ...
   * @throws ArithmeticException if the result exceeds the internal capacity
   */
  def plus(period: ReadablePeriod): LocalDateTime = {
   ...
   */
  def property(fieldType: DateTimeFieldType): LocalDateTime.Property = {
    if (fieldType == null) {
      throw new IllegalArgumentException("The DateTimeFieldType must not be null")
    }
    ...
    return new LocalDateTime.Property(this, fieldType.getField(getChronology))
  }
   ...
   */
  def centuryOfEra: LocalDateTime.Property = {
   ...
   * @return the year of era property
   */
  def yearOfCentury: LocalDateTime.Property = {
   ...
   * @return the year of era property
   */
  def yearOfEra: LocalDateTime.Property = {
   ...
   */
  def weekyear: LocalDateTime.Property = {
   ...
   */
  def monthOfYear: LocalDateTime.Property = {
   ...
   */
  def weekOfWeekyear: LocalDateTime.Property = {
   ...
   */
  def dayOfYear: LocalDateTime.Property = {
   ...
   */
  def dayOfMonth: LocalDateTime.Property = {
   ...
   */
  def dayOfWeek: LocalDateTime.Property = {
   ...
   */
  def hourOfDay: LocalDateTime.Property = {
   ...
   */
  def minuteOfHour: LocalDateTime.Property = {
   ...
   */
  def secondOfMinute: LocalDateTime.Property = {
   ...
   */
  def millisOfSecond: LocalDateTime.Property = {
   ...
   */
  def millisOfDay: LocalDateTime.Property = {
